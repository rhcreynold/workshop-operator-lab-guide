...
start_local() {
  ...
  docker run -d -e WORKSHOP_NAME=$WORKSHOP_NAME -e PRIVATE_IP=$PRIVATE_IP -e STUDENT_NAME=$STUDENT_NAME -p 8080:8080 quay.io/$QUAY_USER/operator-workshop-lab-guide-$WORKSHOP_NAME &> $TMP_FILE
  if [ $? -eq 0 ]; then
    LAB_CONTAINER=$(cat $TMP_FILE | cut -c1-12)
